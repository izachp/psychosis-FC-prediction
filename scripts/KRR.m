function KRR(indir, cbig, preproc, parc, group)
...
% 2019) available at https://github.com/ThomasYeoLab/CBIG/tree/master/utilities/matlab/predictive_models/KernelRidgeRegression
    ...
    setup_param.y = readtable(strcat(indir, '/outcomes_observed', group, '.csv')); % csv/txt of predictors with headings (should be in the same subject order as the FC)
    ...
    setup_param.feature_mat = load(strcat(indir, '/FC_', preproc, '_', parc, group, '.mat')); % FC is saved in the .mat as roi x roi x subjects
    ...
    outdir = strcat(indir, '/', preproc, '_', parc, group);
    ...
    for s=1:length(seeds)
        load(sprintf('final_result_rep%d.mat', s));
        results(s)=mean(optimal_acc);
    end
    
    % Save predicted-observed correlations for all KRR repeats
    cd(indir)
    filename=strcat('r_vals_', preproc, '_', parc, group, '.txt');
